...
# # Logistic regression of gene expression to predict viral presence
# last modified: Friday, Dec 8th, 2023
...
def extract_training_data_new(host_adata,virus_adata_,
    ...
    if equal_pos_neg == True:
        ...
        if control == 'equalprop':
            ...
            virus_low_ = virus_low_[np.random.choice(np.arange(len(virus_low_)),size=n_high_ct, replace=False)]
...
def one_hot_covariates(adata,covariate_list,return_sparse=True):
    ...
    for cov in covariate_list[1:]:
        covariate_array = np.concatenate((covariate_array,np.array(pd.get_dummies(adata.obs[cov]))),axis=1)
...
if covariates_kind == 'none':
    ...
    covariates_array = one_hot_covariates(host_adata,['donor_animal','dpi_clean_merged'])
    num_covariates = covariates_array.shape[1]

num_genes = host_adata.shape[1]

result_dict = {'num_training' : np.ones(len(top_viruses)),
...
for i,v in enumerate(top_viruses):
    ...
    X,Y,training_idx = extract_training_data_new(host_adata,virus_adata[:,v],
                          covariate_array = covariates_array,
                          return_idx = True)
    ...
    # train model
    print(len(Y),'length of X, and sum:,',np.sum(np.array(Y)))
    model= LogisticRegression(random_state=0,penalty=regularization,solver='liblinear').fit(X, Y)
    
    X_test = host_adata[~host_adata.obs.unique_bc.isin(training_idx),:].X
    if covariates_kind != 'none':
        covariates_test = covariates_array[~host_adata.obs.unique_bc.isin(training_idx),:]
        X_test = np.array(scipy.sparse.hstack((X_test,covariates_test)).todense())
    Y_test = np.array(virus_adata[~host_adata.obs.unique_bc.isin(training_idx),v].X.todense()).flatten()
    ...
    X_test = np.array(host_adata[host_adata.obs.unique_bc.isin(virus_adata_test.obs.unique_bc),:].X.todense())
    if covariates_kind != 'none':
        covariates_test = covariates_array[host_adata.obs.unique_bc.isin(virus_adata_test.obs.unique_bc),:]
        X_test = np.array(scipy.sparse.hstack((X_test,covariates_test)).todense())
